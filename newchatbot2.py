...
def generate_response(prompt, topic, conversation_id, counter, last_message):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # Check for similarity with last AI1 message
            if last_ai1_message and is_too_similar(output, last_ai1_message):
                output = utils.get_prompt(topic)
                print("ATTENTION: Repeated response from newchatbot2.py.")
...
def main():
    parser = argparse.ArgumentParser(description='Chatbot 2')
    ...
    response = generate_response(args.prompt, args.topic, args.conversation_id, counter, last_ai1_message)
    ...
    save_message_to_postgres(args.conversation_id, "AI2", response)

    # Publish message to MQTT
    mqtt_client.publish(f'conversations/{args.conversation_id}', json.dumps({"speaker": "AI2", "message": response}))

    # Update MQTT feeds to 'stop' after speaking
    mqtt_client.publish("AI2/status", 'stop')
